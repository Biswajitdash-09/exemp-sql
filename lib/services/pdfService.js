...
export async function generateVerificationReportPDF(verificationData, employeeData) {
  try {
    ...
    autoTable(pdf, {
      ...
      body: [
        [
          ...
          'Date:', new Date(verificationData.verifiedAt).toLocaleDateString()
        ],
        [
    ...
    // Employee Details Table using autoTable
    const employeeRows = [
      ['Employee ID', employeeData.employeeId, 'Date of Joining', formatDate(employeeData.dateOfJoining)],
      ['Full Name', employeeData.name, 'Date of Leaving', formatDate(employeeData.dateOfLeaving)],
      ['Entity Name', employeeData.entityName, 'Designation', employeeData.designation],
      ['Department', employeeData.department || 'N/A', 'Exit Reason', employeeData.exitReason],
      ['F&F Status', { content: employeeData.fnfStatus || 'N/A', styles: { fontStyle: 'bold' } }, '', '']
    ];
    ...
    autoTable(pdf, {
      ...
      columnStyles: {
        0: { fontStyle: 'bold', width: 35, fillColor: [249, 250, 251] },
        1: { width: 50 },
        2: { fontStyle: 'bold', width: 35, fillColor: [249, 250, 251] },
        3: { width: 50 }
      },
      body: employeeRows
    });
    ...
    autoTable(pdf, {
      startY: yPosition,
      margin: { left: margin, right: margin },
      head: [['Field', 'Provided Data', 'Company Records', 'Status']],
      body: comparisonRows,
      theme: 'striped',
...
export async function generateAppealResponsePDF(appealData) {
  ...
  try {
    ...
    pdf.setFontSize(16);
    pdf.setTextColor(0);
    pdf.text('APPEAL RESPONSE', margin, y);
    y += 10;

    pdf.setFontSize(11);
    pdf.text(`Appeal ID: ${appealData.appealId}`, margin, y); y += 7;
