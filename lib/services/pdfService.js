...
export async function generateVerificationReportPDF(verificationData, employeeData) {
  try {
    ...
    const comparisonRows = verificationData.comparisonResults.map(result => {
      let verifierVal = result.verifierValue || 'N/A';
      let companyVal = result.companyValue || 'N/A';

      if (result.label === 'Entity Name') {
        verifierVal = formatEntityName(verifierVal);
        companyVal = formatEntityName(companyVal);
      }

      return [
        result.label,
        { content: verifierVal, styles: { textColor: result.isMatch ? 0 : [220, 38, 38] } }, // Red if mismatch
        companyVal,
        {
