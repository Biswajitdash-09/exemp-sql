...
export async function uploadFileLocally(file, folder = 'uploads') {
  ...
  // Return URL that will be accessible via Next.js static serving
  const publicUrl = `/uploads/${folder}/${filename}`;

  console.log(`[FILE] Local upload saved: ${publicUrl}`);

  return {
    filename: filename,
    localPath: filePath,
    s3Url: publicUrl, // Use relative URL for local files
    originalName: file.name,
    mimeType: file.type,
    size: file.size,
    isLocal: true
